...
logger = get_logger('predly.api.simulation')
...
def get_graph_entities(graph_id: str):
    ...
    Only returns nodes that match predefined entity types (nodes with Labels beyond just Entity)
    ...
    Request (JSON):
        {
            ...
            "graph_id": "predly_xxxx",    // optional, fetched from project if not provided
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    # Required file list (scripts excluded; they are in backend/scripts/)
    required_files = [
        "state.json",
    ...
    try:
        ...
        try:
            ...
            filtered_preview = reader.filter_defined_entities(
                ...
                enrich_with_edges=False  # Don't fetch edge info to speed up
            )
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Iterates through the reports directory, finds reports matching simulation_id,
...
def get_simulation_history():
    ...
    Used for homepage historical project display, returns a rich simulation list
    including project names, descriptions, and other detailed info.
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Get associated project's file list (up to 3 files)
...
def start_simulation():
    ...
    About the force parameter:
        - When enabled, stops and clears run logs if the simulation is running or completed
    ...
    About enable_graph_memory_update:
        - When enabled, all Agent activities (posts, comments, likes, etc.) are updated to the Zep graph in real time
        - Allows the graph to "remember" the simulation process for later analysis or AI conversation
...
def get_run_status_detail(simulation_id: str):
    ...
    Used by the frontend to display real-time activity feed.
    ...
    try:
        ...
        # recent_actions shows only the current latest round's content from both platforms
...
def get_simulation_timeline(simulation_id: str):
    ...
    Used by the frontend to display progress bar and timeline view.
...
def interview_agent():
    ...
    (simulation loop complete and in waiting-for-command mode)
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment is not running or has been closed. Please ensure the simulation has completed and is in waiting-for-command mode."
            }), 400
...
def close_simulation_env():
    ...
    Sends a close-environment command to the simulation, causing it to exit waiting-for-command mode gracefully.
    
    Note: This differs from /stop, which forcibly terminates the process.
