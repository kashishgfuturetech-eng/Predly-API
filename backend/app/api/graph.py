...
def generate_ontology():
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Ontology generation failed: {traceback.format_exc()}")  # ADD
            raise 
...
def build_graph():
    ...
    try:
        ...
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph is already being built. Please do not resubmit. Add force: true to force rebuild",
        ...
        # Get configuration
        graph_name = data.get('graph_name', project.name or 'Predly Graph')
        ...
        def build_task():
            build_logger = get_logger('predly.build')
            try:
