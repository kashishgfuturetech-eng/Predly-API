...
logger = get_logger('predly.zep_entity_reader')
...
class ZepEntityReader:
    ...
    3. Retrieve related edges and connected node info for each entity
    """
    ...
    def _call_with_retry(
        ...
        Args:
            ...
            max_retries: maximum retry attempts (default 3, i.e. up to 3 tries)
    ...
    def get_node_edges(self, node_uuid: str) -> List[Dict[str, Any]]:
        """
        Get all edges related to a specific node (with retry)
    ...
    def filter_defined_entities(
        ...
        Filter logic:
        - If a node's Labels contains only "Entity", it does not match any predefined type — skip it
        - If a node's Labels contains labels beyond "Entity" and "Node", it matches a predefined type — keep it
        ...
        # get all edges (used for relationship lookup)
        all_edges = self.get_all_edges(graph_id) if enrich_with_edges else []
        
        # build a UUID-to-node-data map
        ...
        for node in all_nodes:
            ...
            # filter logic: Labels must contain at least one label besides "Entity" and "Node"
