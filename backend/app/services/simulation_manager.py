...
Uses preset scripts + LLM-based intelligent config generation
"""
...
logger = get_logger('predly.simulation')
...
class SimulationManager:
    ...
    # directory for simulation data storage
    ...
    def prepare_simulation(
        ...
        3. Use LLM to intelligently generate simulation config parameters (timing, activity, post frequency, etc.)
        ...
        try:
            ...
            if progress_callback:
                progress_callback(
                    "reading", 100, 
                    f"complete，total {filtered.filtered_count} entities",
            ...
            if filtered.filtered_count == 0:
                ...
                state.error = "no entity matching the criteria was found，please check if graph construced correctly"
                self._save_simulation_state(state)
                return state
            ...
            # Reddit is already saved in real-time during generation; this save ensures completeness
            if progress_callback:
                progress_callback(
            ...
            # ========== Stage 3: LLM-based intelligent config generation ==========
            if progress_callback:
                progress_callback(
                    "generating_config", 0, 
            ...
            if progress_callback:
                progress_callback(
                    ...
                    "Invoking LLM to generate config...",
                    current=1,
                    total=3
                )
            ...
            # simulation_runner launches them from scripts/ when starting the simulation
    ...
    def get_run_instructions(self, simulation_id: str) -> Dict[str, str]:
        ...
        return {
            ...
            "instructions": (
                f"1. activate the conda environment: conda activate Predly\n"
                f"2. run the simulation (script are located in {scripts_dir}):\n"
                f"   - runninf Twitter indepandantlu: python {scripts_dir}/run_twitter_simulation.py --config {config_path}\n"
                f"   - running Reddit: python {scripts_dir}/run_reddit_simulation.py --config {config_path}\n"
                f"   - parallel execution on dual platform: python {scripts_dir}/run_parallel_simulation.py --config {config_path}"
            )
        }